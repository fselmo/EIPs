...
For most opcodes (`SLOAD`, `BALANCE`, `EXTCODESIZE`, `EXTCODEHASH`, `EXTCODECOPY`), gas is fully determinable pre-state. The following opcodes have post-state gas calculations and require pre-state validation to pass before state access (gas constants as defined in [EIP-2929](./eip-2929.md)):
...
| `SSTORE` | More than `GAS_CALL_STIPEND` available (> 2300 gas) |
...
Post-state costs (e.g., `GAS_NEW_ACCOUNT` for calls to empty accounts, `GAS_SELF_DESTRUCT_NEW_ACCOUNT` for beneficiaries) do not affect BAL inclusion since the target has already been accessed.
...
`SSTORE` performs an implicit read of the current storage value for gas calculation. The > 2300 gas check prevents this state access when operating within the call stipend. If `SSTORE` fails this check, the storage slot MUST NOT appear in `storage_reads` or `storage_changes`.
