...
For opcodes that access state, gas checks are divided into two categories:

- **Pre-state checks**: Gas costs determinable without reading from the state trie (e.g., memory expansion, base opcode cost, warm/cold access costs)
- **Post-state checks**: Gas costs that require reading from the state trie (e.g., checking if an account is alive, resolving [EIP-7702](./eip-7702.md) delegations)

Pre-state gas checks MUST pass before any state trie access occurs. If a pre-state check fails, the target resource (address or storage slot) is never accessed and MUST NOT be included in the BAL. This principle applies to all opcodes that access state.

The following constants are used in the gas checks below (as defined in [EIP-2929](./eip-2929.md)):

| Constant | Value | Description |
|----------|-------|-------------|
| `COLD_ACCOUNT_ACCESS_COST` | 2600 | Cost of accessing a cold account (not previously accessed in the transaction) |
| `WARM_STORAGE_READ_COST` | 100 | Cost of accessing a warm account (previously accessed in the transaction) |
| `GAS_CALL_VALUE` | 9000 | Additional cost for value transfer in call operations |
| `GAS_SELF_DESTRUCT` | 5000 | Base cost of `SELFDESTRUCT` operation |
| `GAS_CALL_STIPEND` | 2300 | Gas stipend provided to the callee on value-transferring calls |

#### Address Inclusion

The following opcodes must pass pre-state gas validation before the target address is accessed and thus included in the BAL:

| Instruction | Target | Pre-state Gas Check |
|-------------|--------|---------------------|
| `CALL` | Target address | `access_cost` + `memory_expansion` + `GAS_CALL_VALUE` (if value > 0) |
| `CALLCODE` | Code address | `access_cost` + `memory_expansion` + `GAS_CALL_VALUE` (if value > 0) |
| `DELEGATECALL` | Code address | `access_cost` + `memory_expansion` |
| `STATICCALL` | Target address | `access_cost` + `memory_expansion` |
| `SELFDESTRUCT` | Beneficiary | `GAS_SELF_DESTRUCT` + `access_cost` |

Where:
- `access_cost`: `COLD_ACCOUNT_ACCESS_COST` if target is cold, otherwise `WARM_STORAGE_READ_COST`
- `memory_expansion`: Gas cost to expand memory for input/output regions

Note: Additional post-state gas costs may apply after state access (e.g., `GAS_NEW_ACCOUNT` if target is not alive, `GAS_SELF_DESTRUCT_NEW_ACCOUNT` if beneficiary is not alive). These do not affect BAL inclusion since the target has already been accessed.

**Example**: A `CALL` to a cold address with value > 0 and memory expansion of 100 gas requires: `2600 + 100 + 9000 = 11700` gas. If only 11000 gas remains, the operation fails before state access and the target address MUST NOT appear in the BAL.

#### EIP-7702 Delegation

For call operations targeting an address with an [EIP-7702](./eip-7702.md) delegation, a second pre-state gas check occurs before the delegation target is accessed:

| Phase | Target | Pre-state Gas Check |
|-------|--------|---------------------|
| 1. Initial | Call target address | (as per table above) |
| 2. Delegation | Delegation target address | `access_cost` for delegation target |

The delegation target's `access_cost` is checked after reading state to resolve the delegation, but before the delegation target address is marked as accessed. If the first check passes but the second fails, the call target address is included in the BAL but the delegation target address MUST NOT be included.

#### Storage Slot Inclusion

`SSTORE` performs an implicit `SLOAD` to read the current value before writing. A pre-state gas check occurs before this state access:

| Instruction | Target | Pre-state Gas Check |
|-------------|--------|---------------------|
| `SSTORE` | Storage slot | More than `GAS_CALL_STIPEND` (> 2300 gas) |

This check prevents `SSTORE` from accessing state when operating within the 2300 gas call stipend. If `SSTORE` fails this check, the storage slot MUST NOT appear in `storage_reads` or `storage_changes`.
